...
public partial class MainWindow: Gtk.Window
{
    ...
    [UI] Button OpenFileButton;
    [UI] Button SaveFileButton;

    [UI] Button ZoomInButton;
    [UI] Button ZoomOutButton;
        ...
        : base(handle)
    {
        ...
        // Button Setup Start //
        OpenFileButton.Image = new Gtk.Image(Stock.Open, IconSize.Button);
        OpenFileButton.Label = string.Empty;
        OpenFileButton.Clicked += OnOpenFileButtonClicked;

        SaveFileButton.Image = new Gtk.Image(Stock.Save, IconSize.Button);
        SaveFileButton.Label = string.Empty;
        SaveFileButton.Clicked += OnSaveFileButtonClicked;

        ZoomInButton.Image = new Gtk.Image(Stock.ZoomIn, IconSize.Button);
        ZoomInButton.Label = string.Empty;
        ZoomInButton.Clicked += OnZoomInButtonClicked;

        ZoomOutButton.Image = new Gtk.Image(Stock.ZoomOut, IconSize.Button);
        ZoomOutButton.Label = string.Empty;
    ...
    protected void OnOpenFileButtonClicked(object sender, EventArgs e)
    {
        ...
        if (fsDialog.Run() == (int)ResponseType.Accept)
        {
            ...
            {
                currentFile = new BLP(File.ReadAllBytes(currentFilePath));
